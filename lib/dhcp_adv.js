...
DHCP.send=(dhcptype,TID,clientip,nextip,serverip)=>{
    ...
    //サーバー側バインド用IPが設定されているばあい、サーバー側MACアドレスを指定
    if(serverip){
        var macaddr=util.getInterfaces()
        av.setUint32(28,0x605718f5)//mac address
        av.setUint16(32,0x367e)// also
    }else{
        av.setUint32(28,0x0)//mac address
        av.setUint16(32,0x0)
    }
    ...
    /**
     * MACアドレス文字列を書き込む
     * @param {string} mac macaddr文字列
     * @param {DataView} av 書き込みたいバッファ
     * @param {offset} [offset] avのオフセット
     */
