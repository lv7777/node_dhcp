...
util.isUsed=(ip)=>{
    ...
    // function setMAC(mac,av,offset = 28){
    //     const arr=mac.split(":")
    //     while(arr.length){
    //         const arrint=parseInt(arr.shift(),16)
    //         av.setUint8(offset,arrint)
    //         offset++
    //     }
    // }
/**
 * MACアドレス文字列を書き込む
 * @param {string} str macaddr文字列
 * @param {int} [ofset] avのオフセット
 * @param {DataView} av 書き込みたいバッファ
 */
util.setMAC=function(str,offset,dv){
    if(/([0-9a-fA-F]+:){5,6}/i.test(str)){
        ...
        for(var i in arr){
            ...
            if(inta>256){
                throw new Error("mac address is too big!!!!!")
            }
            dv.setUint8((offset-0)+(i-0),inta)
            console.log((offset-0)+(i-0)+" is "+arr[i])
        }
    }else if(/([0-9a-fA-F]+-){5,6}/i.test(str)){
        console.log("the str is sepalated - str")
        var arr=str.split("-")
        console.log(arr)
        for(var i in arr){
            ...
            if(inta>256){
                ...
                console.log(inta)
                dv.setUint8((offset-0)+(i-0),arr[i])
            }
        }
    }
}
